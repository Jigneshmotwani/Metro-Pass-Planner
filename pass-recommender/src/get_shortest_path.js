...
// const dest = "Osaka-Umeda"
const dest = "Kawaramachi"
const pass = "Kansai Thru" // or "JR" or "Both"
  ...
  try {
    ...
    for (let i = 0; i < routeLength; i++) {
      ...
      if (currentRailway === null || railway !== currentRailway) {
        if (!linesTaken.includes(railway)) {
          linesTaken.push(railway);
          transferStationsID.push(relationStartID);
        }
        currentRailway = railway;
      }

      if (i === routeLength - 1) {
        transferStationsID.push(relationEndID);
      }
    }
