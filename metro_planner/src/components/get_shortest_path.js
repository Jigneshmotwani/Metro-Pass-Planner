...
// test inputs
const start = 'Osaka-Umeda';
const dest = 'Sakurai';
const pass = 'Both'; // or "JR" or "Both"

// inputs startName, destName, pass (string type, either "Kansai Thru Pass", "JR Pass", or "Both")
...
async function get_shortest_route(startName, destName, pass) {
  ...
  if (pass === 'Kansai Thru Pass') {
    ...
    console.log('Error: Invalid pass input.');
    return [[], []];
  }

  const testquery = `
    MATCH p = shortestPath((start:Station {name: '${startName}'})-[r:Connected*]-(end:Station {name: '${destName}'}))
    WHERE ALL(r IN relationships(p) WHERE r.Operator IN [${availOperators
      .map((op) => `'${op}'`)
      .join(', ')}])
    RETURN p
  `;
...
/* get details of all stations
   returns a list 
    where list[i] is a dictionary containing details of station i
      dictionary include:
      'name', 'lat', 'lng' */
async function get_all_station_details() {
  // local database
  ...
  try {
    ...
    for (let i = 0; i < result.records.length; i++) {
      var stationNode = result.records[i].get(0).properties;
...
async function get_station_details(name_ls) {
  // local database
  // const uri = 'neo4j://localhost:7687'
  // const user = "neo4j"
  // const password = "12345678"
  ...
  try {
    for (let i = 0; i < name_ls.length; i++) {
      ...
      // info retrieval and unpacking
      query =
        "MATCH (n:Station {name: '" + name_ls[i] + "'})-[r]-() RETURN n,r";
      result = await session.run(query);
