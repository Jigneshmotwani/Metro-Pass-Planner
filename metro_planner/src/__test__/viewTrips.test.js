...
import ViewTripsPage from './mockViewTripsPage';

describe('ViewTripsPage Integration Tests', () => {
  // Mock Server
  const server = setupServer(
    rest.get('http://localhost:4000/api/gettrips', (req, res, ctx) => {
      return res(
        ctx.json([
          {
  ...
  test('properly opening and closing the trip modal', async () => {
    render(<ViewTripsPage />);
  ...
  test('properly deletes the trip card', async () => {
    server.use(
      rest.delete(
        'http://localhost:4000/api/removetrip/:id',
        (req, res, ctx) => {
          return res(
            ctx.status(200),
            ctx.json({ message: 'Trip removed successfully!' })
          );
        }
      )
    );
    ...
    await waitFor(() => {
      // Simulate clicking the faDumpster icon to remove the trip
      fireEvent.click(screen.getByTestId('remove-trip-0'));

      // Assert that the trip is indeed removed from the page
      waitFor(() => {
        expect(
          screen.getByText('Kobe Haborland to Hotarugaike')
